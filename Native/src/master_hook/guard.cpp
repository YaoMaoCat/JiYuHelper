...
// Hooks for MasterHelper.exe:
// - TerminateProcess (block process killing)
// - CreateProcessW (prevent spawning new processes)
// - TDProcHookEnableTerminate (prevent enabling termination)
// - BeginSimulate / StopSimulate (network block control)
// - DeviceIoControl (block IOCTL to TDNetFilter.sys)
...
static BOOL WINAPI ProxyTerminateProcess(HANDLE hProcess, UINT exitCode) {
    ...
    Log("[Master] Blocked TerminateProcess(PID=%lu)", pid);
    ...
    LPCWSTR dir, LPSTARTUPINFOW si, LPPROCESS_INFORMATION pi)
{
    ...
    Log("[Master] Blocked CreateProcessW: %ws", app ? app : cmd);
...
static void __cdecl ProxyTDProcHookEnableTerminate(BOOL enable) {
    ...
    Log("[Master] Blocked TDProcHookEnableTerminate(%d)", enable);
...
static void __cdecl ProxyBeginSimulate(DWORD param) {
    ...
    Log("[Master] Blocked BeginSimulate(param=%lu) - internet stays unblocked", param);
...
static void __cdecl ProxyStopSimulate(DWORD param) {
    ...
    Log("[Master] StopSimulate(param=%lu) - allowing", param);
    ...
    LPVOID in, DWORD inSize, LPVOID out, DWORD outSize, LPDWORD ret, LPOVERLAPPED ov)
{
    ...
    // Block IOCTL to TDNetFilter driver
    ...
    if (len > 0) {
        ...
        if (path.find(L"tdnetfilter") != std::wstring::npos) {
            Log("[Master] Blocked DeviceIoControl to TDNetFilter (code=0x%lX)", code);
...
void InstallMasterProcGuard() {
    ...
    Log("[Master] ProcGuard hooks installed (mask=0x%llX)", mask);
    ...
    if (cfg.enableNetSimBlock) {
        ...
        if (hTDM) {
            ...
            if (StopSim) {
                StopSim(0);
                Log("[Master] Called StopSimulate(0) - network unblocked on init");
            }
        }
    }
}
