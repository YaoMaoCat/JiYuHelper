...
// Blocking these IOCTLs keeps filtering off; ClearNetFilter() (see
// core/netfilter.cpp, also compiled into bypass_master as SYSTEM) actively
// resets the driver state on inject/enable.
// ============================================================================
    ...
    LPVOID in, DWORD inSize, LPVOID out, DWORD outSize, LPDWORD ret, LPOVERLAPPED ov)
{
    ...
    if (IsTdNetFilterHandle(hDevice) && IsFilterEnableCode(code)) {
        Log("[NFG] Blocked DeviceIoControl to TDNetFilter (code=0x%lX)", code);
...
void InstallNetFilterGuard() {
    ...
    InstallHooks(hooks);
    Log("[NFG] NetFilter guard installed");
}
