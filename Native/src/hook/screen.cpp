...
// Hook LibJPEG20!EncodeToJPEGBuffer to inject a fake screen image.
// Log analysis showed:
//   Thumbnail: 80x60, stride=240 (80*3 BGR24), pixel format=BGR24
// Corrected signature (7 params): input, w, h, stride, output, outSize, quality
...
// Cached rescaled images per target size (key = packed W|H|Stride)
struct CacheKey {
    int w, h, stride;
...
void ReloadScreenFake() {
    ...
    LoadFakeImage();
    if (g_origBmp)
        Log("[Screen] fake screen reloaded: %dx%d", g_origBmp->GetWidth(), g_origBmp->GetHeight());
    else
        Log("[Screen] fake screen reload FAILED");
}
...
static void LoadFakeImage() {
    ...
    Log("[Screen] Loading fake image: %ws", cfg.screenPngPath);
    ...
    if (!g_origBmp || g_origBmp->GetLastStatus() != Ok) {
        Log("[Screen] FAILED to load screen.png");
        delete g_origBmp;
        g_origBmp = nullptr;
        return;
    }

    Log("[Screen] screen.png loaded: %dx%d", g_origBmp->GetWidth(), g_origBmp->GetHeight());
}
...
static BYTE* GetRescaled(int w, int h, int stride) {
    ...
    if (scaled->LockBits(&r, ImageLockModeRead, PixelFormat24bppRGB, &bd) == Ok) {
        ...
        // GDI+ PixelFormat24bppRGB = BGR in memory? No, it's actually RGB.
        // GDI+ uses "RGB" byte order (R=0, G=1, B=2) but in memory it depends.
        // Actually Windows GDI+ PixelFormat24bppRGB stores BGR (same as GDI)
        // Let's check: first pixel = B,G,R in our test.
        // If the thumbnail comes out as wrong colors, swap B/R below.
    ...
    Log("[Screen] Rescaled to %dx%d stride=%d (%d bytes)", w, h, stride, bufSize);
    return buf;
}
                              ...
                              void* output, int* outSize, int quality)
{
    ...
    if (!logged) {
        logged = true;
        Log("[Screen] EncodeToJPEGBuffer: %dx%d stride=%d quality=%d in=0x%p out=0x%p outSizeP=0x%p",
            w, h, stride, quality, input, output, outSize);
    }
