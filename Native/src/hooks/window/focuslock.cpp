...
// NOT for broadcast windows, child controls, or the main window.
...
static BOOL WINAPI ProxySetForegroundWindow(HWND hwnd) {
    ...
    if (IsOurProcess(hwnd)) {
        JY_LOGT(3000, "focus", "SetForegroundWindow blocked: hwnd=0x%p", hwnd);
        return TRUE;
    }
...
static BOOL WINAPI ProxyBringWindowToTop(HWND hwnd) {
    ...
    if (IsOurProcess(hwnd)) {
        JY_LOGT(3000, "focus", "BringWindowToTop blocked: hwnd=0x%p", hwnd);
        return TRUE;
    }
...
static HWND WINAPI ProxySetActiveWindow(HWND hwnd) {
    ...
    if (IsOurProcess(hwnd)) {
        JY_LOGT(3000, "focus", "SetActiveWindow blocked: hwnd=0x%p", hwnd);
        return hwnd;
    }
...
// ---- SetWindowPos ----
// Only redirect HWND_TOP/TOPMOST for ":20b:" black screen windows.
    ...
    int x, int y, int cx, int cy, UINT flags)
{
    ...
    if (IsOurProcess(hwnd) && (insertAfter == HWND_TOP || insertAfter == HWND_TOPMOST)) {
        if (IsBlackScreenForSWP(hwnd)) {
            JY_LOGT(3000, "focus", "SetWindowPos topmost -> notopmost (black screen)");
...
void InstallFocusLockHook() {
    ...
    InstallHooks(hooks);
    JY_LOGI("focus", "focus lock hooks installed (SWP+SFW+BTT+SAW)");
}
