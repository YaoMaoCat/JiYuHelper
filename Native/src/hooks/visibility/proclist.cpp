...
static void InitWhitelist() {
    ...
    JY_LOGI("proclist", "whitelist built: %zu entries", g_whitelist.size());
}
...
// ---- Process32FirstW / Process32NextW (toolhelp enumeration) ----
...
// ---- EnumProcesses (psapi enumeration, primary for StudentMain) ----
// Filter non-whitelisted processes out of the returned PID list.
static BOOL WINAPI ProxyEnumProcesses(DWORD* pids, DWORD cb, DWORD* needed) {
    ...
    for (DWORD i = 0; i < count; i++) {
        ...
        if (pid == 4) {
            pids[writeIdx++] = pid; // System process
    ...
    JY_LOGD("proclist", "EnumProcesses filtered: %lu -> %lu processes", count, writeIdx);
...
void InstallProcListHook() {
    ...
    InstallHooks(hooks);
    JY_LOGI("proclist", "hooks installed (P32 + EP), %zu whitelisted", g_whitelist.size());
}
